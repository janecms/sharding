...
- 前提：已确认目标IP地址，和目标端口(默认是80).
	1.构造http请求报文:可以是get,post,put,delete...
	2.根据目标地址，目标端口，源地址，源端口创建TCP socket连接，并设置TCP参数。
	3. HTTP报文封装到TCP报文中，并将TCP报文通过socket进行传输。
	2.1 如果目标地址与原地址在同一子网下(同一子网内主机通过交换机连接)，发生泛洪，服务器会收到到TCP报文。
		其他机器目标地址与本机地址不匹配，将报文丢弃。
	2.2 如果目标地址与原地址不在同一子网下，报文转发到网关节点。网关是连接不同子网的设备。
	

## 25.synchronize 关键字与 volatile 关键字的作用和区别。
两者都是并发编程的2个重要的关键字。一般
## 26. 说一下内连接，左连接，右连接。
